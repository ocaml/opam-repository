...
shopt -s nullglob
for llvm_config in llvm-config-$version llvm-config${version//./} llvm-config-mp-$version llvm${version}-config $brew_llvm_config llvm-config-${version}-32 llvm-config-${version}-64 llvm-config; do
